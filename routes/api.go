...
func registerEvents(api fiber.Router, db *database.Database) {
	events := api.Group("/events")

	events.Get("/", Controller.GetAllEvents(db))
	events.Get("/:id", Controller.GetEvent(db))
	events.Post("/", Controller.AddEvent(db))
	events.Put("/:id", Controller.EditEvent(db))
	events.Delete("/:id", Controller.DeleteEvent(db))
}
